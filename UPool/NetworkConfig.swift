...
// MARK: - NetworkConfig: NSObject, NSCoding

class NetworkConfig: NSObject, NSCoding {
    ...
    convenience init?(dictionary: [String:AnyObject]) {
        ...
        if let imageDictionary = dictionary[NetworkClient.JSONResponseKeys.ConfigImages] as? [String:AnyObject],
            let urlString = imageDictionary[NetworkClient.JSONResponseKeys.ConfigBaseImageURL] as? String,
            let secureURLString = imageDictionary[NetworkClient.JSONResponseKeys.ConfigSecureBaseImageURL] as? String,
            let posterSizesArray = imageDictionary[NetworkClient.JSONResponseKeys.ConfigPosterSizes] as? [String],
            let profileSizesArray = imageDictionary[NetworkClient.JSONResponseKeys.ConfigProfileSizes] as? [String] {
    ...
    private func updateConfiguration() {
        ...
        NetworkClient.sharedInstance().getConfig() { (didSucceed, error) in
            if let error = error {
    ...
    class func unarchivedInstance() -> NetworkConfig? {
        
        if FileManager.default.fileExists(atPath: _fileURL.path) {
            return NSKeyedUnarchiver.unarchiveObject(withFile: _fileURL.path) as? NetworkConfig
        } else {
            return nil
        }
    }
